...
object JsonSerializer {
    ...
    @Serializable data class EntryDTO(
        val indicator: String = "MA", val logicOp: String = "AND",
        val timeframe: String = "CURRENT", val entryType: String = "MARKET",
        // v7 unified comparison
        val comparison: String = "GT", val targetType: String = "FIXED",
        val targetFixed: Double = 0.0, val targetPrice: String = "Close",
        val targetIndicator: String = "MA", val targetBuffer: Int = 0,
        val srcBuffer: Int = 0,
        // indicator params
    ...
    @Serializable data class FusionDTO(
        val enabled: Boolean = false,
        val weights: List<WeightDTO> = listOf(WeightDTO("MA", 1.0), WeightDTO("RSI", 0.5)),
    ...
    @Serializable data class WeightDTO(
        val indicator: String = "MA", val weight: Double = 1.0, val minStrength: Double = 0.0,
    )
    ...
    private fun toEntryDTO(e: EntryCondition) = EntryDTO(
        ...
        // v7 unified comparison
        comparison = e.comparison.name, targetType = e.targetType.name,
        targetFixed = e.targetFixed, targetPrice = e.targetPrice,
        targetIndicator = e.targetIndicator.name, targetBuffer = e.targetBuffer,
        srcBuffer = e.srcBuffer,
        // indicator params
    ...
    private fun fromEntryDTO(d: EntryDTO) = EntryCondition(
        ...
        // v7 unified comparison
        comparison = tryEnum(d.comparison), targetType = tryEnum(d.targetType),
        targetFixed = d.targetFixed, targetPrice = d.targetPrice,
        targetIndicator = tryEnum(d.targetIndicator), targetBuffer = d.targetBuffer,
        srcBuffer = d.srcBuffer,
        // indicator params

...
""")
    ...
    private fun StringBuilder.appendInputs(c: StrategyConfig) {
        ...
        c.entries.forEachIndexed { i, e ->
            ...
            when (e.indicator) {
                ...
                IndicatorType.MA -> append("extern int InpE${n}_P = ${e.fastPeriod}; extern int InpE${n}_MAM = MODE_${e.maMethod.uppercase()}; extern int InpE${n}_AP = PRICE_${e.appliedPrice.uppercase()};\n")
                IndicatorType.RSI -> append("extern int InpE${n}_P = ${e.period}; extern int InpE${n}_AP = PRICE_${e.appliedPrice.uppercase()};\n")
                IndicatorType.STOCH -> append("extern int InpE${n}_K = ${e.kPeriod}; extern int InpE${n}_D = ${e.dPeriod}; extern int InpE${n}_Sl = ${e.slowing};\n")
                ...
                IndicatorType.ADX -> append("extern int InpE${n}_P = ${e.period};\n")
                IndicatorType.SAR -> append("extern double InpE${n}_St = ${e.sarStep}; extern double InpE${n}_Mx = ${e.sarMax};\n")
                IndicatorType.CCI -> append("extern int InpE${n}_P = ${e.period}; extern int InpE${n}_AP = PRICE_${e.appliedPrice.uppercase()};\n")
                IndicatorType.ATR -> append("extern int InpE${n}_P = ${e.atrPeriod};\n")
                IndicatorType.ICHIMOKU -> append("extern int InpE${n}_Tk = ${e.tenkan}; extern int InpE${n}_Kj = ${e.kijun}; extern int InpE${n}_Sk = ${e.senkou};\n")
                ...
                IndicatorType.CANDLE_PATTERN -> append("extern string InpE${n}_Pat = \"${e.candlePattern}\";\n")
                IndicatorType.VOLUME -> append("extern int InpE${n}_P = ${e.period};\n")
                IndicatorType.PRICE -> {} // no indicator params, buffer index in unified model
                else -> append("extern int InpE${n}_P = ${e.period};\n")
            }
            // v7: 统一比较模型
            append("extern int InpE${n}_Buf = ${e.srcBuffer}; // 源缓冲区\n")
            append("extern int InpE${n}_Cmp = ${e.comparison.ordinal}; // 比较 0> 1< 2>= 3<= 4== 5金叉 6死叉\n")
            append("extern int InpE${n}_TgtT = ${e.targetType.ordinal}; // 目标 0固定值 1价格 2指标\n")
            when (e.targetType) {
                TargetType.FIXED -> append("extern double InpE${n}_TgtV = ${e.targetFixed}; // 目标值\n")
                TargetType.PRICE -> append("extern int InpE${n}_TgtP = ${priceOrdinal(e.targetPrice)}; // 0C 1O 2H 3L 4HL2 5HLC3\n")
                TargetType.INDICATOR -> {
                    append("extern int InpE${n}_TgtI = ${e.targetIndicator.ordinal}; // 目标指标\n")
                    append("extern int InpE${n}_TgtB = ${e.targetBuffer}; // 目标缓冲区\n")
                    if (e.targetIndicator == IndicatorType.PRICE)
                        append("extern int InpE${n}_TgtP = ${priceOrdinal(e.targetPrice)};")
                }
            }
            append("\n")
        }
    ...
    private fun StringBuilder.appendSignalFunc(c: StrategyConfig) {
        ...
        c.entries.forEach { e ->
            ...
            when (e.indicator) {
                IndicatorType.ICUSTOM -> {
                    append("bool iCustom${e.id}(string sym) { double v = iCustom(sym, 0, InpE${n}_Indi${if (e.customIndiParams.isNotBlank()) ", ${e.customIndiParams}" else ""}, InpE${n}_Buf, 0); return v > 0; }\n\n")
                }
                IndicatorType.CUSTOM_EXPRESSION -> {
                    append("bool EvalEntry${e.id}(string sym) {\n   return (${e.customExpression.ifBlank { "false" }});\n}\n\n")
                }
                IndicatorType.CANDLE_PATTERN -> {
                    ...
                    append("bool EvalEntry${e.id}(string sym) {\n")
                    val srcCall = mql4SrcCall(e, n, "0")
                    append("   double src0 = $srcCall;\n")
                    val isCross = e.comparison == ComparisonOp.CROSS_ABOVE || e.comparison == ComparisonOp.CROSS_BELOW
                    if (isCross) {
                        val src1Call = mql4SrcCall(e, n, "1")
                        append("   double src1 = $src1Call;\n")
                    }
                    // target
                    when (e.targetType) {
                        TargetType.FIXED -> append("   double tgt = InpE${n}_TgtV;\n")
                        TargetType.PRICE -> append("   double tgt = ${mql4Price(e.targetPrice)};\n")
                        TargetType.INDICATOR -> {
                            append("   double tgt = ${mql4TgtCall(e, n, "0")};\n")
                        }
                    }
                    // comparison
                    when (e.comparison) {
                        ComparisonOp.GT -> append("   return (src0 > tgt);\n")
                        ComparisonOp.LT -> append("   return (src0 < tgt);\n")
                        ComparisonOp.GTE -> append("   return (src0 >= tgt);\n")
                        ComparisonOp.LTE -> append("   return (src0 <= tgt);\n")
                        ComparisonOp.EQ -> append("   return (MathAbs(src0 - tgt) < Point);\n")
                        ComparisonOp.CROSS_ABOVE -> append("   return (src1 <= tgt && src0 > tgt);\n")
                        ComparisonOp.CROSS_BELOW -> append("   return (src1 >= tgt && src0 < tgt);\n")
                    }
                    append("}\n\n")
                }
            }
    ...
    // v7: MQL4 source indicator call helper
    private fun mql4SrcCall(e: EvalEntry, n: Int, bar: String): String {
        return when (e.indicator) {
            IndicatorType.MA -> "iMA(sym, 0, InpE${n}_P, 0, InpE${n}_MAM, InpE${n}_AP, $bar)"
            IndicatorType.RSI -> "iRSI(sym, 0, InpE${n}_P, InpE${n}_AP, $bar)"
            IndicatorType.STOCH -> "iStochastic(sym, 0, InpE${n}_K, InpE${n}_D, InpE${n}_Sl, MODE_SMA, 0, ${stochMode("e", n)}, $bar)"
            IndicatorType.MACD -> "iMACD(sym, 0, InpE${n}_F, InpE${n}_S, InpE${n}_Sig, PRICE_CLOSE, ${macdMode("e", n)}, $bar)"
            IndicatorType.BOLLINGER -> "iBands(sym, 0, InpE${n}_P, InpE${n}_Dv, 0, PRICE_CLOSE, ${bbMode("e", n)}, $bar)"
            IndicatorType.ADX -> "iADX(sym, 0, InpE${n}_P, PRICE_CLOSE, ${adxMode("e", n)}, $bar)"
            IndicatorType.SAR -> "iSAR(sym, 0, InpE${n}_St, InpE${n}_Mx, $bar)"
            IndicatorType.CCI -> "iCCI(sym, 0, InpE${n}_P, InpE${n}_AP, $bar)"
            IndicatorType.ATR -> "iATR(sym, 0, InpE${n}_P, $bar)"
            IndicatorType.ICHIMOKU -> "iIchimoku(sym, 0, InpE${n}_Tk, InpE${n}_Kj, InpE${n}_Sk, ${ichimokuMode("e", n)}, $bar)"
            IndicatorType.ALLIGATOR -> "iAlligator(sym, 0, InpE${n}_J, 13, InpE${n}_T, 8, InpE${n}_L, 5, ${alliMode("e", n)}, MODE_SSMA, PRICE_MEDIAN, $bar)"
            IndicatorType.VOLUME -> "iVolume(sym, 0, $bar)"
            IndicatorType.PRICE -> when {
                e.srcBuffer == 0 -> "iClose(sym, 0, $bar)"
                e.srcBuffer == 1 -> "iOpen(sym, 0, $bar)"
                e.srcBuffer == 2 -> "iHigh(sym, 0, $bar)"
                e.srcBuffer == 3 -> "iLow(sym, 0, $bar)"
                e.srcBuffer == 4 -> "((iHigh(sym, 0, $bar) + iLow(sym, 0, $bar)) / 2)"
                e.srcBuffer == 5 -> "((iHigh(sym, 0, $bar) + iLow(sym, 0, $bar) + iClose(sym, 0, $bar)) / 3)"
                else -> "iClose(sym, 0, $bar)"
            }
            else -> "0"
        }
    }

    // v7: MQL4 target indicator call helper
    private fun mql4TgtCall(e: EvalEntry, n: Int, bar: String): String {
        if (e.targetType != TargetType.INDICATOR) return "0"
        return when (e.targetIndicator) {
            IndicatorType.MA -> "iMA(sym, 0, InpE${n}_P, 0, InpE${n}_MAM, InpE${n}_AP, $bar)"
            IndicatorType.RSI -> "iRSI(sym, 0, InpE${n}_P, InpE${n}_AP, $bar)"
            IndicatorType.STOCH -> "iStochastic(sym, 0, InpE${n}_K, InpE${n}_D, InpE${n}_Sl, MODE_SMA, 0, ${stochMode("tgt", n)}, $bar)"
            IndicatorType.MACD -> "iMACD(sym, 0, InpE${n}_F, InpE${n}_S, InpE${n}_Sig, PRICE_CLOSE, ${macdMode("tgt", n)}, $bar)"
            IndicatorType.BOLLINGER -> "iBands(sym, 0, InpE${n}_P, InpE${n}_Dv, 0, PRICE_CLOSE, ${bbMode("tgt", n)}, $bar)"
            IndicatorType.ADX -> "iADX(sym, 0, InpE${n}_P, PRICE_CLOSE, ${adxMode("tgt", n)}, $bar)"
            IndicatorType.SAR -> "iSAR(sym, 0, InpE${n}_St, InpE${n}_Mx, $bar)"
            IndicatorType.CCI -> "iCCI(sym, 0, InpE${n}_P, InpE${n}_AP, $bar)"
            IndicatorType.ATR -> "iATR(sym, 0, InpE${n}_P, $bar)"
            IndicatorType.ICHIMOKU -> "iIchimoku(sym, 0, InpE${n}_Tk, InpE${n}_Kj, InpE${n}_Sk, ${ichimokuMode("tgt", n)}, $bar)"
            IndicatorType.ALLIGATOR -> "iAlligator(sym, 0, InpE${n}_J, 13, InpE${n}_T, 8, InpE${n}_L, 5, ${alliMode("tgt", n)}, MODE_SSMA, PRICE_MEDIAN, $bar)"
            IndicatorType.VOLUME -> "iVolume(sym, 0, $bar)"
            IndicatorType.PRICE -> when {
                e.targetBuffer == 0 -> "iClose(sym, 0, $bar)"
                e.targetBuffer == 1 -> "iOpen(sym, 0, $bar)"
                e.targetBuffer == 2 -> "iHigh(sym, 0, $bar)"
                e.targetBuffer == 3 -> "iLow(sym, 0, $bar)"
                e.targetBuffer == 4 -> "((iHigh(sym, 0, $bar) + iLow(sym, 0, $bar)) / 2)"
                e.targetBuffer == 5 -> "((iHigh(sym, 0, $bar) + iLow(sym, 0, $bar) + iClose(sym, 0, $bar)) / 3)"
                else -> "iClose(sym, 0, $bar)"
            }
            else -> "0"
        }
    ...
    // v7: MQL4 price accessor
    private fun mql4Price(priceType: kotlin.String?): String = when (priceType?.uppercase()) {
        "OPEN" -> "iOpen(sym, 0, 0)"
        "HIGH" -> "iHigh(sym, 0, 0)"
        "LOW" -> "iLow(sym, 0, 0)"
        "HL2" -> "((iHigh(sym, 0, 0) + iLow(sym, 0, 0)) / 2)"
        "HLC3" -> "((iHigh(sym, 0, 0) + iLow(sym, 0, 0) + iClose(sym, 0, 0)) / 3)"
        else -> "iClose(sym, 0, 0)"
    }

    // v7: buffer mode helpers (use srcBuffer for source, targetBuffer for target)
    private fun stochMode(prefix: String, n: Int): String {
        val buf = if (prefix == "e") "InpE${n}_Buf" else "InpE${n}_TgtB"
        return "$buf == 0 ? MODE_MAIN : MODE_SIGNAL"
    }
    private fun macdMode(prefix: String, n: Int): String {
        val buf = if (prefix == "e") "InpE${n}_Buf" else "InpE${n}_TgtB"
        return "$buf == 0 ? MODE_MAIN : MODE_SIGNAL"
    }
    private fun bbMode(prefix: String, n: Int): String {
        val buf = if (prefix == "e") "InpE${n}_Buf" else "InpE${n}_TgtB"
        return "($buf == 0 ? MODE_UPPER : ($buf == 1 ? MODE_LOWER : MODE_MAIN))"
    }
    private fun adxMode(prefix: String, n: Int): String {
        val buf = if (prefix == "e") "InpE${n}_Buf" else "InpE${n}_TgtB"
        return "($buf == 0 ? MODE_MAIN : ($buf == 1 ? MODE_PLUSDI : MODE_MINUSDI))"
    }
    private fun ichimokuMode(prefix: String, n: Int): String {
        val buf = if (prefix == "e") "InpE${n}_Buf" else "InpE${n}_TgtB"
        return "($buf == 0 ? MODE_TENKANSEN : ($buf == 1 ? MODE_KIJUNSEN : ($buf == 2 ? MODE_SENKOUSPANA : ($buf == 3 ? MODE_SENKOUSPANB : MODE_CHIKOUSPAN))))"
    }
    private fun alliMode(prefix: String, n: Int): String {
        val buf = if (prefix == "e") "InpE${n}_Buf" else "InpE${n}_TgtB"
        return "($buf == 0 ? MODE_GATORJAW : ($buf == 1 ? MODE_GATORTEETH : MODE_GATORLIPS))"
    }

    // v7: price ordinal helper
    private fun priceOrdinal(priceType: kotlin.String?): Int = when (priceType?.uppercase()) {
        "OPEN" -> 1; "HIGH" -> 2; "LOW" -> 3; "HL2" -> 4; "HLC3" -> 5
        else -> 0
    }

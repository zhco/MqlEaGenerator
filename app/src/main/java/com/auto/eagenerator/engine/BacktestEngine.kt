...
object BacktestEngine {
    ...
    private fun precompute(candles: List<Candle>, config: StrategyConfig): Buffers {
        ...
        config.entries.forEach { e ->
            when (e.indicator) {
                IndicatorType.MA -> maPeriods += e.fastPeriod to e.maMethod
                IndicatorType.RSI -> rsiPeriods += e.period
                IndicatorType.MACD -> macdSets += Triple(e.fastPeriod, e.slowPeriod, e.period)
            ...
            // v7: also collect for target indicator
            if (e.targetType == TargetType.INDICATOR) {
                when (e.targetIndicator) {
                    IndicatorType.MA -> maPeriods += e.fastPeriod to e.maMethod
                    IndicatorType.RSI -> rsiPeriods += e.period
                    IndicatorType.MACD -> macdSets += Triple(e.fastPeriod, e.slowPeriod, e.period)
                    IndicatorType.BOLLINGER -> bbSets += e.bbPeriod to e.bbDeviation
    ...
    private fun evalSignals(cfg: StrategyConfig, buf: Buffers, i: Int, candles: List<Candle>): Int {
        ...
        val results = cfg.entries.map { e ->
            // get source value at bar i
            val src = btGetSrc(buf, e, i, candles) ?: return@map 0
            // get source at bar i-1 for cross detection
            val isCross = e.comparison == ComparisonOp.CROSS_ABOVE || e.comparison == ComparisonOp.CROSS_BELOW
            val src1 = if (isCross && i > 0) btGetSrc(buf, e, i - 1, candles) else null
            // get target value
            val tgt = when (e.targetType) {
                TargetType.FIXED -> e.targetFixed
                TargetType.PRICE -> btGetPrice(candles, i, e.targetPrice)
                TargetType.INDICATOR -> btGetTgt(buf, e, i, candles) ?: return@map 0
            }
            // compare
            when (e.comparison) {
                ComparisonOp.GT -> if (src > tgt) 1 else 0
                ComparisonOp.LT -> if (src < tgt) 1 else 0
                ComparisonOp.GTE -> if (src >= tgt) 1 else 0
                ComparisonOp.LTE -> if (src <= tgt) 1 else 0
                ComparisonOp.EQ -> if (abs(src - tgt) < 1e-10) 1 else 0
                ComparisonOp.CROSS_ABOVE -> if (src1 != null && src1 <= tgt && src > tgt) 1 else 0
                ComparisonOp.CROSS_BELOW -> if (src1 != null && src1 >= tgt && src < tgt) -1 else 0
            }
        }
    ...
    // v7: get source indicator value for backtest
    private fun btGetSrc(buf: Buffers, e: EntryCondition, i: Int, candles: List<Candle>): Double? {
        return when (e.indicator) {
            IndicatorType.MA -> buf.ma["${e.fastPeriod}_${e.maMethod}"]?.getOrNull(i)
            IndicatorType.RSI -> buf.rsi[e.period]?.getOrNull(i)
            IndicatorType.MACD -> buf.macdMain["${e.fastPeriod}_${e.slowPeriod}_${e.period}"]?.getOrNull(i)
            IndicatorType.BOLLINGER -> {
                when (e.srcBuffer) {
                    0 -> buf.bbUpper["${e.bbPeriod}_${e.bbDeviation}"]
                    1 -> buf.bbLower["${e.bbPeriod}_${e.bbDeviation}"]
                    else -> null
                }?.getOrNull(i)
            }
            IndicatorType.ATR -> buf.atr[e.atrPeriod]?.getOrNull(i)
            IndicatorType.ADX -> buf.adx[e.period]?.getOrNull(i)
            IndicatorType.CCI -> buf.cci[e.period]?.getOrNull(i)
            IndicatorType.PRICE -> btGetPrice(candles, i, when (e.srcBuffer) {
                0 -> "Close"; 1 -> "Open"; 2 -> "High"; 3 -> "Low"
                4 -> "HL2"; 5 -> "HLC3"; else -> "Close"
            })
            else -> null
        }
    }

    // v7: get target indicator value for backtest
    private fun btGetTgt(buf: Buffers, e: EntryCondition, i: Int, candles: List<Candle>): Double? {
        if (e.targetType != TargetType.INDICATOR) return null
        return when (e.targetIndicator) {
            ...
            IndicatorType.ADX -> buf.adx[e.period]?.getOrNull(i)
            IndicatorType.CCI -> buf.cci[e.period]?.getOrNull(i)
            IndicatorType.PRICE -> btGetPrice(candles, i, when (e.targetBuffer) {
                0 -> "Close"; 1 -> "Open"; 2 -> "High"; 3 -> "Low"
                4 -> "HL2"; 5 -> "HLC3"; else -> "Close"
            })
            else -> null
    ...
    // v7: get price value
    private fun btGetPrice(candles: List<Candle>, i: Int, priceType: String): Double {
        val c = candles.getOrNull(i) ?: return 0.0

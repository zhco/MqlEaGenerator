...
enum class IndicatorType(val label: String, val bufferCount: Int = 1, val bufferNames: List<String> = listOf("Main")) {
    MA("MA均线", 1, listOf("Main")),
    RSI("RSI", 1, listOf("Main")),
    STOCH("Stochastic", 2, listOf("Main", "Signal")),
    MACD("MACD", 2, listOf("Main", "Signal")),
    BOLLINGER("布林带", 3, listOf("Upper", "Middle", "Lower")),
    ADX("ADX", 1, listOf("Main")),
    SAR("Parabolic SAR", 1, listOf("Main")),
    CCI("CCI", 1, listOf("Main")),
    ICHIMOKU("一目均衡", 5, listOf("Tenkan", "Kijun", "SenkouA", "SenkouB", "Chikou")),
    ALLIGATOR("鳄鱼线", 3, listOf("Jaw", "Teeth", "Lips")),
    ATR("ATR", 1, listOf("Main")),
    CANDLE_PATTERN("K线形态", 1, listOf("Pattern")),
    VOLUME("成交量", 1, listOf("Volume")),
    PRICE("K线价格", 1, listOf("Price")),
    CUSTOM_EXPRESSION("自定义表达式", 1, listOf("Main")),
    ICUSTOM("自定义指标", 9, listOf("Buf0","Buf1","Buf2","Buf3","Buf4","Buf5","Buf6","Buf7","Buf8")),
}
...
data class EntryCondition(
    val id: Int = 0, val indicator: IndicatorType = IndicatorType.MA,
    ...
    // v7: 统一比较模型
    val srcBuffer: Int = 0,
    val comparison: ComparisonOp = ComparisonOp.GT,
    val targetType: TargetType = TargetType.FIXED,
    val targetFixed: Double = 0.0,
    val targetPrice: String = "Close",
    val targetIndicator: IndicatorType = IndicatorType.MA,
    val targetBuffer: Int = 0,
